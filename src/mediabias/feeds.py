...
def canonical(url):
    p = urlsplit(url)
    query = [(k, v) for k, v in parse_qsl(p.query) if not k.lower().startswith('utm_') and k.lower() not in {'gclid', 'fbclid'}]
...
async def public_url(url, allowed_hosts):
    p = urlsplit(url)
    if p.scheme != 'https' or p.username or p.password or p.port not in {None, 443} or host(url) not in allowed_hosts:
...
async def fetch(client, url, allowed_hosts, state):
    headers = {'User-Agent': 'MediaBiasResearch/0.1 (public RSS only)', 'Accept': 'application/rss+xml, application/atom+xml, application/xml, text/xml'}
    if state.get('etag'):
...
def parse(data, allowed_hosts, clock=None):
    ...
    for entry in feed.entries[:100]:
        ...
        if not stamp or not title or len(text) < 20 or host(url) not in allowed_hosts or urlsplit(url).scheme != 'https':
            rejected += 1
            continue
        ...
        if published < clock - timedelta(days=7) or published > clock + timedelta(hours=6):
            rejected += 1
            continue
        items.append({'title': title[:500], 'text': text[:20000], 'url': canonical(url), 'published_at': published})
    return items[:12], rejected


def tokens(text):
    return set(re.findall(r'\w{3,}', text.replace('İ', 'i').replace('I', 'ı').lower())) - {'için', 'ile', 'bir', 'olan', 'son', 'haber'}


def candidate_event(session, title, published):
    # Conservative candidate grouping; the interface supports human reassignment.
    recent = session.scalars(select(Article).where(Article.published_at >= published - timedelta(hours=48),
        Article.published_at <= published + timedelta(hours=48)).order_by(Article.published_at.desc()).limit(1000)).all()
    wanted, best, score = tokens(title), None, 0.0
    for a in recent:
        t = tokens(a.title)
        similarity = len(t & wanted) / max(1, len(t | wanted))
        if similarity > max(.42, score):
...
class Collector:
    ...
    async def one(self, client, source, feed, allowed):
        ...
        outcome = {**old, 'source_id': source['id'], 'url': url, 'checked_at': now().isoformat(), 'inserted': 0, 'updated': 0}
        try:
            ...
            if data is None:
                ...
                outcome.update(status='eligible_active' if items else 'no_recent_usable_items', error=None,
                    parsed=True, usable_items=len(items), rejected_items=rejected,
                    ...
                    for item in items:
                        ...
                        if article:
                            if article.content_hash != digest:
                                article.title, article.text, article.content_hash = item['title'], item['text'], digest
                                article.fetched_at = now()
                                outcome['updated'] += 1
                            continue
                        # Wire duplicates stay visible across outlets; repeated same-outlet payloads do not.
                        duplicate = session.scalar(select(Article.id).where(Article.source_id == source['id'], Article.content_hash == digest))
                        if duplicate:
                            continue
                        event_id = candidate_event(session, item['title'], item['published_at'])
                        session.add(Article(event_id=event_id, source_id=source['id'], source_name=source['name'],
                            content_scope='feed_excerpt', content_hash=digest, **item))
                        session.flush()
                        outcome['inserted'] += 1
            outcome.update(last_success=now().isoformat(), consecutive_failures=0)
        except httpx.HTTPStatusError as error:
            outcome.update(status='access_restricted' if error.response.status_code in {401, 403} else 'candidate_failed_runtime',
                http_status=error.response.status_code, error='Feed HTTP request was not successful.')
