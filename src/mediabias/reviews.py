...
def export_rows(session):
    ...
    rows = session.execute(select(Decision, Review, Analysis, NewsEvent)
        .join(Review, Decision.review_id == Review.id).join(Analysis, Review.analysis_id == Analysis.id)
        .join(NewsEvent, Analysis.event_id == NewsEvent.id).order_by(Decision.created_at, Decision.id)).all()
    latest = {}
    ...
    for decision, review, analysis, event in latest.values():
        ...
        finding = ground_finding(Finding.model_validate(finding), {a['id']: a for a in analysis.snapshots}).model_dump()
        ...
        yield {'schema_version': '1.0', 'task': 'evidence_scoped_finding_verification',
            'event_id': event.id, 'analysis_id': analysis.id, 'split': split_for(event.id),
            'input': {'articles': analysis.snapshots, 'proposed_finding': original or finding},
            'target': {'verdict': payload['verdict'], 'finding': None if payload['verdict'] == 'reject' else finding,
