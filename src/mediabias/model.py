...
def fingerprint(snapshots, settings, sensitivity):
    body = {'articles': snapshots, 'model': settings.llm_model, 'verifier': settings.verifier_model,
            'endpoint': settings.llm_base_url, 'sensitivity': sensitivity,
            'taxonomy': TAXONOMY_HASH, 'prompt': PROMPT_VERSION}
...
class Analyzer:
    ...
    async def complete(self, messages, model):
        ...
        async with self.gate, httpx.AsyncClient(timeout=120, follow_redirects=False) as client:
            for attempt in range(3):
                response = await client.post(s.llm_base_url.rstrip('/') + '/chat/completions',
    ...
    async def analyze(self, snapshots, sensitivity='sensitive'):
        ...
        aliases = {s: f'Kaynak {i + 1}' for i, s in enumerate(dict.fromkeys(a['source_id'] for a in snapshots))}
        articles = [{'id': a['id'], 'source': aliases[a['source_id']], 'text': a['text'],
                     'content_scope': a['content_scope'], 'truncated': a.get('truncated', False)} for a in snapshots]
        concepts = [{k: e[k] for k in ('id', 'layer', 'operational_definition', 'annotation_boundary')} for e in CONCEPTS.values()]
        setting = ('İnce işaretleri kaçırmamak için makul ama kanıtı eksik bulguları candidate olarak tut.'
                   if sensitivity == 'sensitive' else 'Yalnızca güçlü metinsel desteği olan bulguları tut.')
        messages = [{'role': 'system', 'content': SYSTEM + setting}, {'role': 'user', 'content': json.dumps({
            'schema': Comparison.model_json_schema(), 'concepts': concepts, 'articles': articles}, ensure_ascii=False)}]
        ...
        messages += [{'role': 'assistant', 'content': json.dumps(draft, ensure_ascii=False)},
                     {'role': 'user', 'content': 'Taslağı kaynak metinlerle denetle. Yanlış alıntı, yanlış atıf, aşırı çıkarım ve kaçırılmış ince çerçeveleri düzelt. Tam düzeltilmiş JSON döndür.'}]
