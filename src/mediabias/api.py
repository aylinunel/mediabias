...
def create_app(settings=None):
    ...
    def config():
        return {
            "mode": settings.mode,
            "auth_required": bool(settings.users),
    ...
    def events():
        with sessions() as session:
            result = session.execute(
                select(NewsEvent, func.count(Article.id), func.count(func.distinct(Article.source_id)))
                .outerjoin(Article, Article.event_id == NewsEvent.id)
    ...
    def event_detail(event_id: str):
        with sessions() as session:
            ...
            articles = session.scalars(
                select(Article).where(Article.event_id == event_id).order_by(Article.id)
            ).all()
            ...
            stale = bool(
                analysis
                and {(a["id"], a["content_hash"]) for a in analysis.snapshots}
                != {(a["id"], a["content_hash"]) for a in snapshots}
            )
            return {
                "id": e.id,
                "title": e.title,
        ...
        with sessions.begin() as session:
            ...
            for a in body.articles:
                ...
                session.add(
                    Article(
                        event_id=e.id,
                        content_hash=hashlib.sha256((a.title + "\n" + a.text).encode()).hexdigest(),
                        **values,
                    )
                )
                session.flush()
        ...
        async with analysis_lock:
            with sessions() as session:
                ...
                if len(articles) > settings.max_event_articles:
                    raise HTTPException(
                        422, "Olaydaki metin sayısı sınırı aşıyor; olayı daraltın veya sınırı yapılandırın."
                    )
                snapshots = [snapshot(a, settings.max_article_chars) for a in articles]
                key = fingerprint(snapshots, settings, body.sensitivity)
                existing = session.scalar(
                    select(Analysis).where(Analysis.event_id == event_id, Analysis.cache_key == key)
                )
            ...
            try:
                ...
                raise HTTPException(
                    502,
                    "Model analizi tamamlanamadı veya kanıt doğrulaması başarısız. Yapılandırmayı ve sağlayıcı durumunu kontrol edin.",
                ) from error
            with sessions.begin() as session:
                current = session.scalars(select(Article).where(Article.event_id == event_id)).all()
                if {(a.id, a.content_hash) for a in current} != {
                    (a["id"], a["content_hash"]) for a in snapshots
                }:
    ...
    async def collect():
        ...
        if settings.collector_mode != "api":
            raise HTTPException(409, "Toplama ayrı işçi sürecinde çalışıyor; kaynak tablosunu yenileyin.")
        try:
    ...
    def reviews(analysis_id: str):
        with sessions() as session:
            ...
            return {
                ...
                "decisions": [
                    {"id": d.id, "review_id": d.review_id, "editor": d.editor, "notes": d.notes}
    ...
    def review(analysis_id: str, body: ReviewInput, user=Depends(identity)):
        with sessions.begin() as session:
            ...
            r = Review(
                analysis_id=analysis_id, finding_id=body.finding_id, reviewer=user["id"], payload=payload
            )
            session.add(r)
            session.flush()
            return {"id": r.id}
    ...
    def export():
        ...
        return Response(
            ...
            headers={"Content-Disposition": 'attachment; filename="reviewed_annotations.jsonl"'},
        )
