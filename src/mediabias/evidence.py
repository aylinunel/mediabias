...
def ground_finding(finding: Finding, articles):
    ...
    if finding.concept_id in {"contextual_omission", "one_sided_sourcing", "selective_statistics"}:
        ...
        if not (complete and comparative):
            ...
            finding.missing_context_tr = (
                "Tam, karşılaştırılabilir metin ve ilgili karşı kanıt gerekli. " + finding.missing_context_tr
            )
...
def validate_comparison(value, snapshots):
    ...
    if any(a.get("truncated") for a in snapshots):
        result.limitations_tr.append(
            "Bazı metinler uzunluk sınırında kesildi; bulunmayan bağlam hakkında sonuç çıkarılamaz."
        )
    if any(a["content_scope"] != "full_text" for a in snapshots):
        result.limitations_tr.append(
            "Özetler tam haberi temsil etmeyebilir; bağlamın yokluğu kanıtlanmış değildir."
        )
    return result
