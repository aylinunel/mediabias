...
def convert(path):
    ...
    for row in list(workbook['Canlı Akışlar'].values)[1:]:
        ...
        sources[slug]['feeds'].append({'url': url, 'category': category,
            'provided_status': status, 'provided_latest': str(last) if last else None,
            'enabled': status == 'canlı' and url.startswith('https://')})
    return {'version': 1, 'observed_at': '2026-09-08T22:38:00Z',
